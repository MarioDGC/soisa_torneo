<?php
    ...
    function registrarPuntos() {
        ...
        $sqlEquipo1 = "SELECT id_equipo_1 FROM enfrentamiento WHERE id_enfrentamiento = $idEnfrentamiento";
        $queryEquipo1 = mysqli_query($cnx, $sqlEquipo1);
        $rowsEquipo1 = mysqli_num_rows($queryEquipo1);

        if ($rowsEquipo1 > 0) {

            $arrEnfrentamiento = mysqli_fetch_array($queryEquipo1);
            $id_equipo1_sql = $arrEnfrentamiento['id_equipo_1'];

            // comprobar si el $idEquipo id_equipo_1 o id_equipo_2
            $columna = ($idEquipo == $id_equipo1_sql) ? "equipo_1_$setColumna" : "equipo_2_$setColumna";
            ...
            $data['result'] = 'warning: No se pudo localizar ID del enfrentamiento. Volver a intentar.';
        }
